...
SENSITIVE_FIELDS = {
    'password', 'new_password', 'code', 'id_token', 'token', 'hash'
}
...
def send_verification_code():
    ...
    try:
        ...
        logger.info("📧 Получены данные: %s", redact_sensitive(data))
        
        if not data:
            return jsonify({'error': 'Данные не предоставлены'}), 400
        
        email = data.get('email', '').strip().lower()
        logger.info(f'📩 Email для регистрации: {email}')
        ...
        logger.info('🔢 Сгенерирован код подтверждения для %s', email)
...
def verify_and_register():
    ...
    try:
        ...
        email = data.get('email', '').strip().lower()
        logger.info(f'📩 Email для восстановления: {email}')
...
def send_password_reset_code():
    ...
    """
    try:
        logger.info('🔔 Вызвана функция send_password_reset_code')
        data = request.get_json()
        logger.info("📧 Получены данные: %s", redact_sensitive(data))
        ...
        logger.info(f'👤 Найден пользователь: {user.username if user else "Не найден"}')
        if not user:
        ...
        # Генерируем код
        code = generate_verification_code()
        logger.info('🔢 Сгенерирован код восстановления для %s', email)
        ...
        if send_password_reset_email(email, code):
            logger.info("Код восстановления пароля отправлен на %s", email)
            return jsonify({
                'success': True,
                'message': 'Код восстановления отправлен на почту'
            }), 200

        logger.error("Не удалось отправить код восстановления на %s", email)
        return jsonify({'error': 'Ошибка отправки email. Проверьте настройки почты.'}), 500
...
def delete_account():
    ...
    try:
        ...
        logger.info(f"Deleted account: {username} (ID: {user.id})")
