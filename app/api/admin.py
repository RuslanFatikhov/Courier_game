...
from app.utils.city_helper import load_cities_config
import logging
...
def get_system_status():
    ...
    try:
        ...
        import os
        config_path = 'data/cities/cities_config.json'
        data_files_status = {
            'cities_config': os.path.exists(config_path),
            'missing_files': []
        }
        config = load_cities_config(config_path)
        for city in config.get('cities', []):
            ...
            if not os.path.exists(restaurants_file):
                data_files_status['missing_files'].append(restaurants_file)
            if not os.path.exists(buildings_file):
                data_files_status['missing_files'].append(buildings_file)
